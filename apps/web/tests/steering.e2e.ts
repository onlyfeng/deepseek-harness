...
describe('web e2e: empty-draft Cmd+Enter steers the whole queue', () => {
  ...
  it.skipIf(MODE === 'record')('queues two messages, then flushes both with an empty-draft Cmd+Enter', async () => {
    ...
    expect(await page.locator('[data-queue-dock]').count()).toBe(0)
    // Wait for think to settle, then for the question composer, before the
    // mid snapshot. captureStableAria polls until two frames match; on a
    // quiet runner the composer arrives inside that poll, so a pre-composer
    // golden is not a stable milestone.
