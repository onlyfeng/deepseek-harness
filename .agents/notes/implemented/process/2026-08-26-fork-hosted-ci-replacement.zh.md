...
- `artifacts` 运行 `pnpm run check:ci:artifacts`（官方 `pnpm run build`，随后 publint、built-package invariants、`verify-node-next-types` 和 built-bin smokes），然后运行 `pnpm run duplication` 以及带 `DSH_DOC_TYPECHECK_USE_BUILD_OUTPUT=1` 的 `doc-typecheck:contracts-ready`。在该官方构建之后，它用 `--with-deps` 安装 Playwright Chromium（与上游 consumer 作业的托管路径相同），并运行 `DSH_SNAPSHOT=replay pnpm run test:web:ci`。托管 `ubuntu-latest` 有 4 个 CPU，因此 `DSH_WEB_SNAPSHOT_WORKERS` 为 `2`（上游在 16 核池上使用 `6`）。该作业不运行 `check:ci:consumers`：那个聚合还会重复 snapshots 与 node-compat。
...
本 fork 上的拉取请求与 `dev/compat` 推送等待 `fork checks passed`，而不是上游的 `all checks passed` 聚合。该替代方案覆盖每一项托管的上游作业，以及原先由私有 runner 拥有的静态、快照、产物与 Playwright Web 快照聚合。穷尽单元覆盖率、原生 Windows 以及持久 pwsh PTY ACP 场景仍是本地或上游私有 runner 上的证据；仅限于那些套件的回归在此仍可能合并。托管 Ubuntu 镜像包含 `pwsh`，因此一次性 `pwshOnly` ACP header 必须与当前的 job 工具和 pwsh 工具描述保持一致。
