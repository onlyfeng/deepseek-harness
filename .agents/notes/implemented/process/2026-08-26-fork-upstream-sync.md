...
`origin/master` fast-forwards to `upstream/master` when it is an ancestor of that commit, and the job fails without force-pushing when it is not. When `dev/compat` does not contain `upstream/master`, the job opens or reuses a `master` → `dev/compat` pull request. GitHub's conflict-resolution UI and any push to that PR's head commit onto `master` and break the fast-forward-only mirror. Resolve conflicts on a branch created from `dev/compat` by merging `origin/master`, open a PR into `dev/compat`, and close the sync PR.
...
**Resolve the sync PR's conflicts in the GitHub UI, or push a merge commit to `master`.** Either writes fork-only history onto the mirror. The next scheduled fast-forward then fails because `origin/master` is no longer an ancestor of `upstream/master`.
