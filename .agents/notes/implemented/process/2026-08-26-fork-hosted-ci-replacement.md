...
- `artifacts` runs `pnpm run check:ci:artifacts` (official `pnpm run build`, then publint, built-package invariants, `verify-node-next-types`, and built-bin smokes), then `pnpm run duplication` and `doc-typecheck:contracts-ready` with `DSH_DOC_TYPECHECK_USE_BUILD_OUTPUT=1`. Those are the consumer-lane artifact checks that `check:ci:snapshot` does not own. The job does not run `check:ci:consumers`: that aggregate also repeats snapshots and node-compat and starts Playwright web snapshots.
...
**Run `check:ci:consumers` as the replacement for the private consumer job.** That aggregate repeats `snapshotGate()` and `check:node-compat`, which this workflow already owns as separate jobs, and it starts Playwright web snapshots that need a Chromium install the hosted substitute does not provide.
