...
- `artifacts` runs `pnpm run check:ci:artifacts` (official `pnpm run build`, then publint, built-package invariants, `verify-node-next-types`, and built-bin smokes), then `pnpm run duplication` and `doc-typecheck:contracts-ready` with `DSH_DOC_TYPECHECK_USE_BUILD_OUTPUT=1`. After that official build it installs Playwright Chromium with `--with-deps` (the same hosted path the upstream consumer job uses) and runs `DSH_SNAPSHOT=replay pnpm run test:web:ci`. Hosted `ubuntu-latest` has 4 CPUs, so `DSH_WEB_SNAPSHOT_WORKERS` is `2` (upstream uses `6` on the 16-core pool). The job does not run `check:ci:consumers`: that aggregate also repeats snapshots and node-compat.
