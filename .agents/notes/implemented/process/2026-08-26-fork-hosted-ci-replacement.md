...
- `snapshots` prepares bubblewrap with `scripts/prepare-ci-bubblewrap.sh`, then runs the same official `pnpm run build` plus `DSH_EXAMPLE_MODE=lib` `test:snapshot` graph as `snapshotGate()`, with vitest `-t '!persistent-pwsh-tool-turn'` so the hosted persistent PTY run is omitted. Lib-mode Web snapshots request `workspace-write` and refuse to run unconfined; ACP and headless sessions without a usable sandbox backend inject a `danger-full-access` runtime-context snapshot that fixtures recorded with a sandbox do not contain. Hosted Ubuntu images ship `pwsh`, so one-shot `pwshOnly` scenarios run and their headers must match current tool descriptions. The job sets `DSH_SNAPSHOT_MAX_CONCURRENCY=1` so the DeepSeek SSE keep-alive comment test is not starved by parallel snapshot files. GitHub-hosted persistent PTYs return shell-init scrollback instead of command output; the committed fixture is not rewritten to that result. Fixture guards still cover `snapshots/session/persistent-pwsh-tool-turn/`. Local `pnpm run test:snapshot` still executes the persistent scenario. The skip lives in [`fork-ci.yml`](../../../../.github/workflows/fork-ci.yml) only: `snapshot.yml` rejects unknown fields, and editing `session-snapshot` would conflict on every sync.
...
- `python-runtime` calls the shared [single-executable builder](../../../../.github/workflows/build-exe-for-python-sdk.yml) for `node24-linux-x64` with `ci: true`, matching the [required Python runtime job](../../archived/testing/2026-08-12-required-python-runtime-pull-request-ci.md).
...
**Add a skip field to `snapshot.yml` or `session-snapshot`, or refresh the persistent-pwsh golden from hosted PTY output.** The YAML parser rejects unknown fields, and `session-snapshot` is an upstream package that would conflict on every sync. The live hosted result is shell-init scrollback, not `PWSH_OK`; recording that output would pin a broken PTY. Changing prompt settlement is product terminal behavior; the same hosted PTY class is why coverage and the terminal/shell unit suites stay omitted. One-shot `pwsh-tool-turn` still runs. Fixture guards still cover the committed files. Local `pnpm run test:snapshot` still executes the persistent scenario.
